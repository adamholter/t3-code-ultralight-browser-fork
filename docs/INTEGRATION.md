...
The GitHub release tarball remains the canonical package source. A published-release workflow checks out the exact tag, repeats the packed consumer and artifact gates, audits production dependencies, reconstructs the versioned and stable archives, verifies their npm publication shape with a dry run, creates GitHub provenance attestations, and replaces the release assets with the verified outputs. Public npm publishing is optional and runs only when repository authentication is configured; integrations never need to depend on that additional channel.
