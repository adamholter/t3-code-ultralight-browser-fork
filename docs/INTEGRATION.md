...
Every recipe includes `workspace: { default: "bridge", overrideEmbedded: false }` unless a programmatic recipe author intentionally passes `cwd` to `createIntegrationRecipe()`. Setup never does so because the standalone bridge already owns the verified workspace default. Its sibling trusted `lifecycle` receipt provides structured `{ command, args, cwd }` operations for installed and zero-install hosts. Run `lifecycle.ensure` before the web host: it pins the already-resolved numeric port, workspace, real Codex executable, and origin policy rather than re-running automatic selection. `status` and `stop` use the same exact port. Failed setup receipts return `lifecycle: null` and make no process change.
...
The packaged `integration.json` advertises automatic setup but describes 4174 as its portable default. A running bridge's `/api/integration` and `/integration.json` responses are materialized for its resolved numeric port; use the live response when generating links or lifecycle commands for a nondefault bridge. Runtime setup commands are pinned to that exact port rather than selecting again. Installer authors can import `createIntegrationRecipe()` and `materializeRuntimeIntegrationContract()` from `t3-code-ultralight-browser-fork/integration`.

The GitHub release tarball remains the canonical package source. A published-release workflow checks out the exact tag, repeats the packed consumer and artifact gates, audits production dependencies, reconstructs the versioned and stable archives, verifies their npm publication shape with a dry run, creates GitHub provenance attestations, and replaces the release assets with the verified outputs. Public npm publishing is optional and requires both repository authentication and the explicit `NPM_PUBLISH_ENABLED=true` variable; inherited credentials alone cannot activate it, and integrations never need to depend on that additional channel.

Use `t3-code-ultralight status --json` to inspect the default-port standalone bridge without starting Codex, or pass the numeric port returned by automatic setup. `serve` and `start` are idempotent only for an identical version, allowed-origin set, normalized workspace fingerprint, and Codex-binary fingerprint, making repeated agent setup safe without silently inheriting broader access, another project's directory, or another CLI installation. On an explicit port, a conflicting version, workspace, binary, any missing or extra origin, invalid port, or unrelated listener fails with an actionable message. Automatic selection skips those conflicts without changing them. Status returns only workspace and binary fingerprints; the trusted setup/start receipt includes the selected values. Use `t3-code-ultralight stop [--port PORT] [--json]` before an upgrade, origin, workspace, or binary change; it validates the service identity and reported PID, waits for shutdown, and is safe to repeat. Pass `--reuse-origin-superset` only when the invoking host intentionally accepts every additional origin already configured; the JSON receipt exposes the accepted extras.
