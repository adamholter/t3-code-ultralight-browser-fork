...
```bash
npx --yes 'https://github.com/adamholter/t3-code-ultralight-browser-fork/releases/latest/download/t3-code-ultralight-browser-fork.tgz?v=0.53.0' setup --mode iframe --port auto --allow-origin http://localhost:3000 --json
```
...
```bash
npx --yes 'https://github.com/adamholter/t3-code-ultralight-browser-fork/releases/latest/download/t3-code-ultralight-browser-fork.tgz?v=0.53.0' setup --mode custom --delivery hosted --port auto --allow-origin http://localhost:3000 --json
```
...
```bash
npx --yes 'https://github.com/adamholter/t3-code-ultralight-browser-fork/releases/latest/download/t3-code-ultralight-browser-fork.tgz?v=0.53.0' start --allow-origin http://localhost:3000
```
...
```bash
npm install 'https://github.com/adamholter/t3-code-ultralight-browser-fork/releases/latest/download/t3-code-ultralight-browser-fork.tgz?v=0.53.0'
...
Requirements: a currently supported Node.js release (22+) and a working `codex` CLI login. CI runs the full source and packed-consumer checks on Ubuntu, macOS, and Windows at the Node 22 floor, plus Node 24 and 26 on Ubuntu. Every OS job also launches a disposable Codex-compatible app-server process through the same resolver used in production; Windows uses a real `.cmd` shim.
