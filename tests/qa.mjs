...
console.log(JSON.stringify({ threadCount, modelCount, userMessageCount, sidebarX: sidebarBox?.x, overflow, consoleErrors }, null, 2));
await browser.close();
await deleteSmokeThread(smokeText);

if (threadCount < 1 || modelCount < 1 || userMessageCount !== 1 || !sidebarBox || sidebarBox.x !== 0 || overflow || consoleErrors.length) process.exit(1);
